...
add_action('admin_enqueue_scripts', function ($hook) {
    ...
    if (isset($_GET['page']) && strpos($_GET['page'], 'ai-site-chatbot') !== false) {

        wp_enqueue_style(
            'aisc-admin-style',
            AISC_URL . 'assets/css/admin.css',
            [],
            AISC_VERSION
        );
        ...
        wp_enqueue_script(
            'aisc-admin-indexer',
            AISC_URL . 'assets/js/admin-indexer.js',
            ['jquery'],
            AISC_VERSION,
            true
        );
...
// Instellingen-link in plugin overzicht
add_filter(
    'plugin_action_links_' . plugin_basename(__FILE__),
    function ($links) {
        $settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ai-site-chatbot&tab=instellingen')) . '">Instellingen</a>';
...
// INCLUDES
// ─────────────────────────────────────────
require_once AISC_PATH . 'includes/admin/settings-page.php';
...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_localize_script('aisc-chat-script', 'aiscChat', [
        'ajaxurl'      => admin_url('admin-ajax.php'),
...
// ─────────────────────────────────────────
// ALWAYS USE PROXY IN FREE VERSION
// ─────────────────────────────────────────
